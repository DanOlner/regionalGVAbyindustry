...
We then pick a place to take an initial look at - **Liverpool City Region (LCR)**. This will be the main place on the plot, with others to compare to. We also order the sectors by LCR's LQ, so they're ordered in the plots by LCR.
...
2. Use plotly to make an interactive plot, so hovering over lines will show what places are interactively. (That can't be included on this page, but will run in RStudio, and there's an [online version here](https://danolner.github.io/regionalGVAbyindustry/GVA_percent_liverpool_city_region_motorvehicles.html).)
